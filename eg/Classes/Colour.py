...
class Colour:
    ...
    @staticmethod
    def SetProgramColours(frame):
        return

        import eg

        def iter_child(parent):
            if isinstance(parent, eg.HeaderBox):
                text = parent.text

                text = text.replace(
                    'bgcolor="' + parent.GetBackgroundColour().GetAsString(wx.C2S_HTML_SYNTAX),
                    'bgcolor="' + wx.Colour(60, 60, 60).GetAsString(wx.C2S_HTML_SYNTAX)
                )

                text = text.replace(
                    'text="' + parent.GetForegroundColour().GetAsString(wx.C2S_HTML_SYNTAX),
                    'text="' + wx.Colour(195, 195, 195).GetAsString(wx.C2S_HTML_SYNTAX)
                )
                parent.text = text
