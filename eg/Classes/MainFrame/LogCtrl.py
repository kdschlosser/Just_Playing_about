...
class LogCtrl(wx.ListCtrl):
    ...
    """
    def __init__(self, parent, event_string):
        self.maxlength = 2000
        ...
        wx.CallAfter(self.SetData, eg.log.GetData(event_string=event_string))
    ...
    def OnCmdClearLog(self, dummyEvent=None):
        ...
        self.DeleteAllItems()
        self.data.clear()
        eg.log.ClearData(self.event_string)
        self.FocusLastItem()
        self.Scroll()
        self.Refresh()
    ...
    def SetData(self, data):
        ...
        if self.event_string in eg.document.visibleLogItems:
            self.EnsureVisible(eg.document.visibleLogItems[self.event_string])
        else:
...
class MainLogCtrl(ultimatelistctrl.UltimateListCtrl):
    """
    Implementation of a ListCtrl with a circular buffer.
    """
    def __init__(self, parent):
        ...
        ultimatelistctrl.UltimateListCtrl.__init__(
            ...
            agwStyle=(
                ...
                ultimatelistctrl.ULC_SINGLE_SEL |
                ultimatelistctrl.ULC_SORT_ASCENDING |
                ultimatelistctrl.ULC_BORDER_SELECT |
                ultimatelistctrl.ULC_HEADER_IN_ALL_VIEWS
            )
        )
        if eg.config.useFixedFont:
            df = self.GetFont()
    ...
    def _create_event_log_pane(self, event_string):
        if event_string not in self.event_panes:
            parent = self.GetParent()

            logCtrl = LogCtrl(parent, event_string)
            logCtrl.Freeze()

            logCtrl.SetDateLogging(self.logDates)
            logCtrl.SetTimeLogging(self.logTimes)
            logCtrl.SetIndent(bool(self.indent))

            pane = wx.aui.AuiPaneInfo()
            pane.Name(event_string)
            pane.Left()
            pane.MinSize((280, 300))
            pane.MaximizeButton(True)
            pane.MinimizeButton(True)
            pane.CloseButton(True)
            pane.Caption(" " + event_string)
            parent.auiManager.AddPane(logCtrl, pane)
            parent.auiManager.Update()
            logCtrl.Thaw()

            self.event_panes[event_string] = (logCtrl, pane)

        self.event_panes[event_string][1].Show()
    ...
    def OnDoubleClick(self, event):
        ...
        if flags & wx.LIST_HITTEST_ONITEM:
            icon, wref = self.GetItemData(item)[1:3]
            if isinstance(wref, eg.EventGhostEvent):
                self._create_event_log_pane(wref.string)
            else:
                node = wref()
                if node is not None and not node.isDeleted:
                    node.Select()
    ...
    def SetData(self, data):
        ...
        for event_string in eg.document.visibleLogItems.keys():
            self._create_event_log_pane(event_string)
    ...
    def SetEventMeter(self, completed):
        thread = threading.current_thread()

        if isinstance(thread, eg.EventGhostEvent):
            for i, item in enumerate(self.data):
                if thread.string in item[0]:
                    break
            else:
                return
            ...
            gauge.SetValue(int(round(completed)))
            self.SetItem(item)
    ...
    def WriteLine(self, line, icon, wRef, when, indent):
        ...
        with self.__lock:
            ...
            if isinstance(wRef, eg.EventGhostEvent):
                event_string = wRef.string

                for i, item in enumerate(self.data):
                    if event_string in item[0]:
                        break
                else:
                    wx.CallAfter(do)
                    event.wait()
                ...
                if gauge is None:
                    gauge = wx.Gauge(self, -1, 100, style=wx.GA_HORIZONTAL | wx.GA_SMOOTH)
                    item.SetWindow(gauge)

                gauge.SetValue(0)
                self.SetItem(item)

                if event_string in self.event_panes:
                    self.event_panes[event_string][0].WriteLine(
                        line,
                        icon,
                        wRef,
                        when,
                ...
                if isinstance(thread, eg.EventGhostEvent):
                    if thread.string in self.event_panes:
                        self.event_panes[thread.string][0].WriteLine(
                            line,
                            icon,
                            wRef,
                            when,
