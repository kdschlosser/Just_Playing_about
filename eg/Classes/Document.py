        ...
        if self.reentrantLock.acquire(False):
            if self.frame is not None:
                if len(self.frame.openDialogs) == 0:
                    ...
                    for key, (event_log, pane) in logCtrl.event_panes.items():
                        if event_log.IsAutoscroll():
                            log_item = 0
                        else:
                            log_item = (
                                event_log.GetTopItem() + event_log.GetCountPerPage()
                            )

                            if log_item:
                                log_item -= 1

                        self.visibleLogItems[event_log.event_string] = log_item
