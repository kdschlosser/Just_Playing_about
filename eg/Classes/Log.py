...
class Log(object):
    ...
    @eg.LogIt
    def GetData(self, numLines=-1, event_string=None):
        if event_string is None:
            data = self.data
        else:
            data = self.logContainers[event_string]
    ...
    def ClearData(self, event_string=None):
        if event_string is None:
            self.data.clear()
        else:
            self.logContainers[event_string].clear()

    def LogEvent(self, event):
        """
        Store and display an EventGhostEvent in the logger.
        """
        _ = self.logContainers[event.string]
    ...
    def Write(self, text, icon, wRef=None):
        ...
        if thread == eg.mainThread:
            data = self.data
        else:
            data = self.logContainers[thread.string]

        when = time()
        for line in lines[:-1]:
            data.append((line, icon, wRef, when, eg.indent))
            self._WriteLine(line, icon, wRef, when, eg.indent)
